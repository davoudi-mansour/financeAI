...
class FinancialTimeSeriesDataset(Dataset):
    def __init__(self, data, window_length, zone_area, stop_area, indexes):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        while candidate < len(self.data):
            ...
            if abs(price_change) >= self.zone_area:
                ...
                flag = self.validate(self.stop_area, last_price, val_sequence, state)
                if all(flag):
                    label = 0 if price_change >= self.zone_area else 3
                else:
                    if flag[1] == True:
                        label = 1
                    elif flag[1] == False:
            ...
            candidate += 1
        return torch.zeros(self.window_length), torch.tensor(1)
...
class CNNBiLSTM(nn.Module):
    def __init__(self, dropout_rate=0.3, num_classes=4):
...
#zone_area = 0.0038
zone_area = 5
stop_area = zone_area / 3
window_length = 6000

total_sequences = len(range(0, len(_15min_samples) - window_length, 30))
train_size = int(total_sequences * 0.8)
val_size = int(total_sequences * 0.1)
test_size = total_sequences - train_size - val_size

indices = list(range(0, len(_15min_samples) - window_length, 30))
#np.random.shuffle(indices)
train_indices = indices[:train_size]
val_indices = indices[train_size:train_size + val_size]
test_indices = indices[train_size + val_size:]

train_dataset = FinancialTimeSeriesDataset(_15min_samples, window_length, zone_area, stop_area, train_indices)
val_dataset = FinancialTimeSeriesDataset(_15min_samples, window_length, zone_area, stop_area, val_indices)
test_dataset = FinancialTimeSeriesDataset(_15min_samples, window_length, zone_area, stop_area, test_indices)
...
plt.show()
#best_model_path = os.path.join(save_dir, "best_model_epoch_2.pt")
